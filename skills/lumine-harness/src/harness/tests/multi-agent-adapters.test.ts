...
import { initializeSessionState, observeHarnessEvent, recordWorkStatus } from "../core/work-status.ts";
import { beginVerificationRun } from "../core/verification.ts";
import { resolveHarnessRuntimeRoot, resolveSkillAssetsRoot } from "../core/runtime-layout.ts";
import type {
...
interface CapabilityProductFixture extends Omit<HarnessAdapterCapability, "capabilities" | "continuation"> {
  capabilities: Record<AdapterCapabilityName, AdapterCapabilityResult>;
  continuation: NonNullable<HarnessAdapterCapability["continuation"]>;
  repositoryTestedBridgeVersion?: string;
  verifiedBridgeVersion?: string;
}

interface CapabilityManifestFixture {
  schemaVersion: number;
  skillSource: string;
  products: Record<HarnessProduct, CapabilityProductFixture>;
}

interface HookConfigFixture {
  hooks: {
    stop?: Array<{ loop_limit: number }>;
    Stop?: Array<{ loop_limit: number }>;
  };
}
...
test("public Stop Policy preserves all six states and deduplicates one status revision", () => {
  ...
  try {
    ...
    const done: HarnessSessionInput = { product: "kimi", sessionId: "done", cwd: root };
...
test("ZCode applies its three-cycle host limit without limiting later user-authorized chains", () => {
  const root = tempHarness();
  const input: HarnessHookInput = { product: "zcode", sessionId: "zcode-limit", cwd: root, event: "stop" };
  try {
    initializeSessionState(root, input);
...
test("repository keeps one canonical Skill source and capability boundaries remain explicit", () => {
  ...
  const capabilityNames = [
    ...
    "work_status_matrix",
    "session_isolation"
  ] as const satisfies readonly AdapterCapabilityName[];
  const results = new Set<CapabilityResult>(["passed", "needs_setup", "not_tested", "not_observable", "not_applicable", "failed"]);
  const evidenceLevels = new Set<CapabilityEvidenceLevel>(["official_declared", "repository_checked", "runtime_observed", "behavior_verified"]);
