...
import { initializeSessionState, observeHarnessEvent, readSessionState, recordUsedSkill } from "../../../core/work-status.ts";
...
export async function handleCodeBuddyHook(raw: UnknownRecord = {}): Promise<HookResult> {
  ...
  if (event === "prompt_submit") {
    ...
    return {
      exitCode: 0,
      stdout: contextOutput("UserPromptSubmit", `${buildSessionStartContext({ ...input, root, prompt })}${required}`)
    };
  }
...
if (process.argv[1] && new URL(import.meta.url).pathname === process.argv[1]) {
  main().catch((error) => {
    process.stderr.write(`codebuddy hook failed: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  });
}
