...
import { getSharedSkill } from "../../../core/skill-catalog.ts";
...
export async function handleDeepSeekHarnessHook(raw: UnknownRecord = {}): Promise<HookResult> {
...
if (process.argv[1] && new URL(import.meta.url).pathname === process.argv[1]) {
  main().catch((error) => {
    process.stderr.write(`deepseek-harness hook failed: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  });
}
