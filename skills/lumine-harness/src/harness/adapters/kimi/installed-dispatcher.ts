...
if (root) {
  const module = await import(pathToFileURL(path.join(root, ".harness", "adapters", "kimi", "hooks", "dispatch.mjs")).href) as {
    handleKimiHook: (raw: unknown) => Promise<{ stdout?: string; stderr?: string; exitCode: number }>;
  };
  const result = await module.handleKimiHook(raw);
