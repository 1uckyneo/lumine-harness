...
export const ADAPTER_CAPABILITIES: readonly AdapterCapabilityName[] = [
...
interface VerificationEvent {
  ...
  decision?: {
    action: HarnessHookDecision["action"];
    disposition: HarnessHookDecision["disposition"] | null;
    workStatus: WorkStatus | null;
    workStatusRevision: number | null;
    continuationRequestId: string | null;
    shouldDeliver: boolean | null;
  };
}
...
type CapabilitySummary = Record<AdapterCapabilityName, AdapterCapabilityResult>;
...
interface VerifyRuntimeOptions {
  verificationRunId?: string;
  maxAgeMs?: number;
}

interface AppendVerificationDetails {
  raw?: UnknownRecord;
...
export interface RuntimeVerificationResult {
  product: HarnessProduct;
  status: "challenge_issued" | "not_tested" | "failed" | "runtime_observed";
...
function readChallenge(dir: string): VerificationChallenge | null {
  const file = path.join(dir, "probe.json");
  if (!existsSync(file)) return null;
  try { return JSON.parse(readFileSync(file, "utf8")) as VerificationChallenge; } catch { return null; }
}

export function beginVerificationRun(
  root: string,
  product: HarnessProduct,
  options: BeginVerificationOptions = {}
...
function activeVerificationRunId(root: string, product: HarnessProduct): string | null {
  const file = activeProbeFile(root, product);
  if (!existsSync(file)) return null;
  try {
    const active = JSON.parse(readFileSync(file, "utf8")) as { product?: HarnessProduct; expiresAt?: string; verificationRunId?: string };
 ...
 */
export function ensureVerificationRun(
  root: string,
  product: HarnessProduct,
  options: BeginVerificationOptions = {}
): RuntimeVerificationResult {
...
export function verificationRunId(input: HarnessHookInput, raw: UnknownRecord = {}, root: string | null = null): string | null {
...
export function appendVerificationEvent(
  ...
  details: AppendVerificationDetails = {}
...
function readEvents(file: string): VerificationEvent[] {
  return readFileSync(file, "utf8").split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line) as VerificationEvent);
}

function candidateRuns(root: string, product: HarnessProduct): CandidateRun[] {
  ...
  return readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => {
    ...
    try {
      ...
      return { runId: entry.name, file, challenge, events: filtered };
    } catch { return null; }
...
function failedEvidence(product: HarnessProduct, message: string): RuntimeVerificationResult {
...
export function verifyRuntimeEvidence(
  ...
  const run = options.verificationRunId
    ...
    : runs.find((item) => item.runId === activeRunId) ?? runs.find((item) => item.challenge);
  if (!run) return { product, status: "not_tested", capabilities: capabilityDefaults(product), messages: ["没有找到有效的安全探针或真实会话事件。"] };
