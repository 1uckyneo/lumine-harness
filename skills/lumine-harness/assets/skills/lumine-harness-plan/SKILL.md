...
2. 读取 ready draft。
3. draft 仍有待确认决策时停止回到 `lumine-harness-draft`。
4. 需要设计确认但 `DESIGN.md` 未 approved，或已批准页面没有完整 `prototypes[]`、handoff 和 design_data 时，停止回到 `lumine-harness-design`。
5. 使用 `lumine-harness-generated` 刷新并复核任务相关索引。
...
- 需要设计确认时链接 `DESIGN.md`，并列出 `prototypes[]` 中已批准页面的 prototype、screenshots、component map、handoff 和 design_data；按页面拆分实施和验收。
