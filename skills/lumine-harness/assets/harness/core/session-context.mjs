...
// Source: skills/lumine-harness/src/harness/core/session-context.ts
import { requireHarnessRoot } from "./root-resolver.mjs";
import { buildSharedSkillCatalog } from "./skill-catalog.mjs";
import { existsSync, readFileSync, readdirSync } from "node:fs";
import path from "node:path";

//#region skills/lumine-harness/src/harness/core/session-context.ts
function activePlans(root) {
	const dir = path.join(root, "docs", "exec-plans", "active");
	if (!existsSync(dir)) return [];
	return readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".md")).filter((entry) => {
	...
	}).map((entry) => `docs/exec-plans/active/${entry.name}`).sort();
}
