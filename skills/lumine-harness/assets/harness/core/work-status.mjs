...
// Source: skills/lumine-harness/src/harness/core/work-status.ts
...
function hashRuntimeIdentifier(value) {
...
function deriveStatusEmissionId(input, state = {}) {
	const raw = input?.raw ?? {};
	const responseId = input.statusEmissionId ?? raw.assistant_message_id ?? raw.assistantMessageId ?? raw.response_id ?? raw.responseId ?? raw.message_id ?? raw.messageId ?? raw.generation_id ?? raw.generationId;
...
function readCurrentSessionPointer(root, product) {
	const file = getCurrentSessionPointerPath(root, product);
...
function listCurrentSessionPointers(root) {
	...
	return readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => {
		try {
			return JSON.parse(readFileSync(path.join(dir, entry.name), "utf8"));
		} catch {
			return null;
		}
	}).filter((item) => Boolean(item?.product && item?.sessionId));
}
function readSessionState(root, product, sessionId) {
...
function writeSessionState(root, product, sessionId, patch = {}) {
	requireIdentity(product, sessionId);
	const file = getSessionStatePath(root, product, sessionId);
	mkdirSync(path.dirname(file), { recursive: true });
	const next = {
...
function initializeSessionState(root, input) {
...
function recordUserTurn(root, input, options = {}) {
	requireIdentity(input.product, input.sessionId);
...
function recordProgressObservation(root, input, options = {}) {
	const state = readSessionState(root, input.product, input.sessionId) ?? initializeSessionState(root, input);
...
function setProgressObservability(root, input, observable = true) {
	const state = readSessionState(root, input.product, input.sessionId) ?? initializeSessionState(root, input);
...
function observeHarnessEvent(root, input, options = {}) {
	...
	const userInitiated = explicitUserInitiated ?? (input.event === "prompt_submit" && !state.pendingContinuationRequestId);
...
function recordUsedSkill(root, input, skill) {
	...
	const next = usedSkills.some((item) => item.name === skill.name && item.source === skill.relativeSource) ? usedSkills : [...usedSkills, {
		name: skill.name,
		source: skill.relativeSource,
