...
// Source: skills/lumine-harness/src/scripts/harness-manager.ts
import { copyFileSync, existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, realpathSync, rmSync, rmdirSync, statSync, writeFileSync } from "node:fs";
...
//#region skills/lumine-harness/src/scripts/harness-manager.ts
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const SKILL_ROOT_CANDIDATE = path.resolve(SCRIPT_DIR, "..");
const SKILL_ROOT = path.basename(SKILL_ROOT_CANDIDATE) === "src" ? path.dirname(SKILL_ROOT_CANDIDATE) : SKILL_ROOT_CANDIDATE;
const PRODUCTS = [
	"codex",
	"qoder",
	"trae",
...
function inspectTarget(targetRoot) {
	...
	const frontend = [
		...
		"svelte.config.js",
		"angular.json"
	].some((name) => names.has(name)) || frontendPackages.some((name) => packageNames.has(name));
...
function sourceMap(_inspect, adapters, modules) {
	const map = new Map();
...
function isProjectOwnedPath(target, project = {}) {
	...
	const extensionPaths = [extensions.projectChecks, extensions.generatedImplementation].filter((value) => typeof value === "string" && Boolean(value.trim())).map((value) => slash(path.normalize(value)));
...
function createProposal(targetRoot, options = {}) {
	...
	for (const [target, source] of sources) {
		...
		let action = !existing ? "create" : previousHash && currentHash === previousHash ? "update-managed" : isProjectOwnedPath(target, currentProject) ? "preserve-project" : "conflict";
	...
	for (const target of ["AGENTS.md", "ARCHITECTURE.md"]) {
		...
		writeSet.push({
			path: target,
			action: existsSync(file) ? "preserve-project" : "create-project-template",
			currentHash: existsSync(file) ? fileHash(file) : null
		});
	}
	...
	writeSet.push({
		path: ".harness/managed.json",
		action: existsSync(path.join(inspect.targetRoot, ".harness", "managed.json")) ? "update-manifest" : "generate-manifest"
	});
	...
	const payload = {
		...
		backupRequired: writeSet.some((item) => [
			"update-managed",
			"update-config",
			"update-manifest",
			"delete-managed"
		].includes(item.action)) || !inspect.hasGit,
		integrity: ""
	};
	payload.integrity = hash(JSON.stringify({
		...payload,
		integrity: undefined
	}));
	return payload;
...
function renderProjectTemplate(name, inspect) {
	...
	return source.replaceAll("{{project_name}}", projectName).replaceAll("{{target_root}}", ".").replaceAll("{{topology}}", inspect.topology).replaceAll("{{implementation_surface}}", `当前工程形态为 ${inspect.topology}，实现入口以工程地图和模块文档为准。`).replaceAll("{{implementation_surfaces}}", inspect.childRepositories.join(", ") || ".").replaceAll("{{repo_rules_entry}}", "AGENTS.md、ARCHITECTURE.md、README.md 与模块级文档").replaceAll("{{directory_map}}", inspect.childRepositories.map((item) => `- \`${item}\`：关联仓库，具体规则以仓库内入口文档为准。`).join("\n") || "- `.`：当前工程根。").replaceAll("{{fact_index_targets}}", "- `AGENTS.md`\n- `ARCHITECTURE.md`\n- `.agents/skills/`\n- `.harness/`\n- `docs/`").replaceAll("{{tech_signals}}", `backend=${Number(inspect.signals.backend)}, frontend=${Number(inspect.signals.frontend)}, db=${Number(inspect.signals.database)}`).replaceAll(/{{[a-z_]+}}/g, "TODO: inspect and document this project-specific value.");
}
...
function pruneEmptyParents(root, start) {
	...
	while (current.startsWith(`${root}${path.sep}`) && current !== root) {
		if ([
			...
			".agents",
			"docs"
		].includes(path.basename(current))) return;
		if (!existsSync(current) || readdirSync(current).length) return;
		rmdirSync(current);
		current = path.dirname(current);
	}
}
function applyProposal(proposalFile) {
	const proposal = readJson(path.resolve(proposalFile));
	verifyProposal(proposal);
	const root = proposal.targetRoot;
	const sources = sourceMap(proposal.inspect, proposal.selectedAdapters, proposal.modules);
	const backupDir = backupFiles(root, proposal.writeSet.filter((item) => [
		"update-managed",
		"update-config",
		"update-manifest",
		"delete-managed"
	].includes(item.action)).map((item) => item.path));
	const files = {};
	for (const item of proposal.writeSet) {
		...
		if (["preserve-project", "unchanged"].includes(item.action)) {
			...
			if (existsSync(file) && statSync(file).isFile() && item.action === "unchanged") files[item.path] = {
				hash: fileHash(file),
				source: "lumine-harness"
			};
			continue;
		}
		const source = sources.get(item.path);
	...
	const project = {
		...
		generatedTargets: Array.isArray(existingProject.generatedTargets) ? existingProject.generatedTargets : ["workspace-index", "repo-doc-index"],
		autonomy: {
	...
	files[".harness/project.json"] = {
		hash: fileHash(projectFile),
		source: "project-config"
	};
	const revision = git(SKILL_ROOT, ["rev-parse", "HEAD"]) || null;
	const sourceDirty = Boolean(git(SKILL_ROOT, [
		"status",
		"--porcelain=v1",
		"-uall"
	]));
...
async function main() {
	...
	if (command === "proposal" || command === "upgrade" && args[0] === "--plan") {
		...
		if (output) {
			...
			writeFileSync(path.resolve(output), `${JSON.stringify(proposal, null, 2)}\n`, "utf8");
		}
...
//#endregion
export { applyProposal, createProposal, inspectTarget };