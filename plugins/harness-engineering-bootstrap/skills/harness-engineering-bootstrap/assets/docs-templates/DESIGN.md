...
| Page ID | 页面 | 目标应用路由 | Prototype | Component Map | Screenshots |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |
...
后续 agent 将原型转成真实页面时必须读取本文件的 `prototypes[]` 和对应 component map，保留 `approved_scope` 内的信息层级和交互路径，并按截图清单确认实现范围。
