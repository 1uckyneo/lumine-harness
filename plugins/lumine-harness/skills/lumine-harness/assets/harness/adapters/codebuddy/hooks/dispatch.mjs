...
// Source: skills/lumine-harness/src/harness/adapters/codebuddy/hooks/dispatch.ts
...
//#region skills/lumine-harness/src/harness/adapters/codebuddy/hooks/dispatch.ts
const EVENT_MAP = {
...
function eventFor(raw) {
	...
	return typeof eventName === "string" ? EVENT_MAP[eventName] : undefined;
}
...
async function handleCodeBuddyHook(raw = {}) {
	...
	if (event === "session_start") {
		return {
			exitCode: 0,
			stdout: contextOutput("SessionStart", buildSessionStartContext({
				...input,
				root
			}))
		};
	...
	if (event === "prompt_submit") {
		...
		return {
			exitCode: 0,
			stdout: contextOutput("UserPromptSubmit", `${buildSessionStartContext({
				...input,
				root,
				prompt
			})}${required}`)
