...
// Source: skills/lumine-harness/src/harness/tests/adoption-manager.test.ts
...
//#region skills/lumine-harness/src/harness/tests/adoption-manager.test.ts
...
test("inspection distinguishes backend, multi-repository, and library or CLI targets", () => {
	...
	try {
		...
		const proposal = createProposal(cliRoot, {
			adapters: "none",
			modules: "auto"
		});
...
test("inspection is read-only inside a Git worktree", () => {
	...
	try {
		...
		const proposal = createProposal(worktreeRoot, {
			adapters: "none",
			modules: "auto"
		});
...
test("adopt requires the reviewed proposal and writes project and managed manifests", () => {
	...
	try {
		...
		assert.deepEqual(project.autonomy, {
...
test("upgrade preserves modified project assets and project config while updating managed Core", () => {
	...
	try {
		...
		assert.deepEqual(upgraded.autonomy, {
			maxContinuationChain: 12,
			noProgressThreshold: 2
		});
...
test("apply rejects target drift and unresolved overlapping files", () => {
	...
	try {
		const driftProposal = createProposal(driftRoot, {
			adapters: "none",
			modules: "workflow"
		});
		...
		const conflictProposal = createProposal(conflictRoot, {
			adapters: "none",
			modules: "workflow"
		});
