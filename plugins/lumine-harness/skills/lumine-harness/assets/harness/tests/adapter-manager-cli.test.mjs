...
// Source: skills/lumine-harness/src/harness/tests/adapter-manager-cli.test.ts
...
import { adapterCheck, adapterStatus, doctorAdapter, formatAdapterResult, runAdapterCommand } from "../adapter-manager.mjs";
...
//#region skills/lumine-harness/src/harness/tests/adapter-manager-cli.test.ts
...
function fixture(selected = ["qoder"]) {
	...
	writeJson(path.join(root, ".harness", "root.json"), {
		schemaVersion: 1,
		kind: "harness-root"
	});
	writeJson(path.join(root, ".harness", "project.json"), {
		schemaVersion: 1,
...
test("adapter status current uses one live pointer and reports ambiguity instead of guessing", () => {
	...
	try {
		...
		const identified = adapterStatus("current", {
			cwd: root,
			env: {}
		});
		...
		const ambiguous = adapterStatus("current", {
			cwd: root,
			env: {}
		});
		...
		const explicit = adapterStatus("current", {
			cwd: root,
			env: {
				HARNESS_PRODUCT: "cursor",
				HARNESS_SESSION_ID: "cursor-session"
			}
		});
...
test("Cursor only requests a concrete trust action when the host explicitly reports a restricted workspace", () => {
	...
	try {
		assert.equal(doctorAdapter("cursor", {
			cwd: root,
			env: {}
		}).status, "repository_ready");
		const restricted = doctorAdapter("cursor", {
			cwd: root,
		...
		assert.equal(doctorAdapter("cursor", {
			cwd: root,
			env: { CURSOR_WORKSPACE_TRUST: "restricted" }
...
test("stale runtime evidence does not turn repository readiness into connection_error", () => {
	...
	try {
		beginVerificationRun(root, "qoder", {
...
test("adapter status supports an explicit product and a stable machine contract", () => {
	const root = fixture(["qoder"]);
	try {
		const output = runAdapterCommand([
			"status",
			"qoder",
			"--json"
		], { cwd: root });
...
test("default Chinese output stays concise and details separates evidence", () => {
	...
	try {
		...
		try {
			...
			const openCodeDetails = formatAdapterResult(adapterStatus("opencode", {
				cwd: openCodeRoot,
				details: true
			}));
...
test("runtime observations are not presented as repository checks", () => {
	...
	try {
		...
		const output = formatAdapterResult(adapterStatus("qoder", {
			cwd: root,
			details: true
		}));
