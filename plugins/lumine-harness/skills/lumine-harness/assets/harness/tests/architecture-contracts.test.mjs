...
// Source: skills/lumine-harness/src/harness/tests/architecture-contracts.test.ts
import { resolveHarnessRuntimeRoot, resolveSkillPackageRoot } from "../core/runtime-layout.mjs";
...
//#region skills/lumine-harness/src/harness/tests/architecture-contracts.test.ts
...
test("dynamic Skill catalog discovers arbitrary project Skills and limits prompt output", async () => {
	...
	try {
		...
		assert.deepEqual(discoverSharedSkills(root).map((skill) => skill.name), [
			"database-migration",
			"lumine-harness-run",
			"security-audit"
		]);
		...
		const catalog = buildSharedSkillCatalog(root, {
			query: "audit run database",
			limit: 2
		});
...
test("canonical Skill reads accept relative and symlinked paths without accepting unrelated files", async () => {
	...
	try {
		...
		assert.equal(sharedSkillReadFromTool(root, {
			tool_name: "Read",
			tool_input: { file_path: ".agents/skills/security-audit/SKILL.md" },
			cwd: root
		})?.name, "security-audit");
		assert.equal(sharedSkillReadFromTool(root, {
			tool_name: "Read",
...
test("parallel sessions keep independent state and continuation is consumed per status revision", async () => {
	const { initializeSessionState, readSessionState, recordWorkStatus, writeSessionState } = await import("../core/work-status.mjs");
	...
	try {
		...
		writeSessionState(root, alpha.product, alpha.sessionId, { expectedSkills: [{
			name: "security-audit",
		...
		assert.throws(() => initializeSessionState(root, {
			product: "codebuddy",
			sessionId: "unknown",
			cwd: root
		}), /explicit product and sessionId/i);
	} finally {
		rmSync(root, {
			recursive: true,
			force: true
...
test("work-status CLI never guesses the active host", async () => {
	...
	const root = tempHarness();
	try {
		assert.throws(() => setCliWorkStatus("done", {
			root,
			cwd: root
		}), /product/i);
		assert.throws(() => setCliWorkStatus("done", {
...
test("runtime probes are opt-in, redact local identity, and report evidence per capability", async () => {
	...
	try {
		...
		assert.equal(appendVerificationEvent(root, {
			...incomplete,
			event: "session_start"
		}), null);
		...
		beginVerificationRun(root, "codebuddy", {
			verificationRunId: "verified",
		...
		appendVerificationEvent(root, {
			...verified,
			event: "session_start"
		}, { observations: ["project_instructions"] });
		appendVerificationEvent(root, {
			...verified,
			event: "prompt_submit"
		});
...
test("runtime verification rejects evidence outside the Harness root", async () => {
	...
	try {
		beginVerificationRun(root, "codebuddy", {
			verificationRunId: "outside-root",
			hostVersion: "test-host-1"
		});
		appendVerificationEvent(root, {
			product: "codebuddy",
			sessionId: "outside-root",
			cwd: outside,
			event: "session_start"
		});
...
test("runtime verification can observe two isolated session streams in one explicit probe", async () => {
	...
	try {
		beginVerificationRun(root, "codebuddy", {
			verificationRunId: "two-sessions",
