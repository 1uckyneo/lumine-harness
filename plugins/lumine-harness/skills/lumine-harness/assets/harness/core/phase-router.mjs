...
//#region skills/lumine-harness/src/harness/core/phase-router.ts
const HARNESS_PHASES = [
	{
		id: "generated",
		skill: "lumine-harness-generated",
		pattern: /\bgenerated\b|生成导航|刷新索引/i
	},
	{
		id: "check",
		skill: "lumine-harness-check",
		pattern: /\bharness[- ]?check\b|全量检查|硬检查/i
	},
	{
		id: "design",
		skill: "lumine-harness-design",
		pattern: /\bdesign\b|设计方向|设计稿|原型设计/i
	},
	{
		id: "draft",
		skill: "lumine-harness-draft",
		pattern: /\bdraft\b|需求草案|需求初稿/i
	},
	{
		id: "run",
		skill: "lumine-harness-run",
		pattern: /\bharness[- ]?run\b|授权\s*run|执行实现|进入实施/i
	},
	{
		id: "plan",
		skill: "lumine-harness-plan",
		pattern: /\bproduct spec\b|\bexec(?:ution)? plan\b|产品规格|执行计划/i
	}
];
function routeHarnessPhase(prompt = "") {
	return HARNESS_PHASES.find((phase) => phase.pattern.test(String(prompt))) ?? null;
}
function expectedSkillPath(root, skill) {
	return path.join(root, ".agents", "skills", skill, "SKILL.md");
}
function recordPromptRoute(root, input, prompt) {
	const phase = routeHarnessPhase(prompt);
	const sharedSkills = discoverSharedSkills(root);
	const explicitNames = [...String(prompt).matchAll(/\$([a-z0-9]+(?:-[a-z0-9]+)*)/gi)].map((match) => match[1].toLowerCase()).filter((name) => sharedSkills.some((skill) => skill.name === name));
...
function pendingExpectedSkills(state = {}) {
	...
	if (state?.expectedSkill && !state.expectedSkillRead) {
		return [{
			name: state.expectedSkill,
			path: state.expectedSkillPath ?? "",
			reason: state.expectedPhase ?? null,
			read: false
		}];
	}
	return [];
}
function markExpectedSkillRead(root, input, state, file) {
	const target = path.resolve(file);
	const expectedSkills = Array.isArray(state?.expectedSkills) ? state.expectedSkills.map((skill) => path.resolve(skill.path) === target ? {
		...skill,
		read: true,
...
function requireExpectedSkillRead(root, input, state, skill, reason = "adapter-routed") {
...
function extractToolInput(raw = {}) {
	const value = raw.tool_input ?? raw.toolInput ?? raw.input;
	return value !== null && typeof value === "object" ? value : {};
}
function toolReadsExpectedSkill(raw, expectedPath) {
...
function toolLoadsExpectedSkill(raw, expectedSkill) {
...
function isMutatingTool(raw = {}) {
	return /^(write|edit|multiedit|notebookedit|bash|shell|terminal|exec|applypatch|apply_patch|write_to_file|search_replace|run_in_terminal|str_replace_editor|run_code)$/i.test(extractToolName(raw).replace(/\s+/g, ""));
}

//#endregion
export { HARNESS_PHASES, expectedSkillPath, extractToolInput, extractToolName, isMutatingTool, markExpectedSkillRead, pendingExpectedSkills, recordPromptRoute, requireExpectedSkillRead, routeHarnessPhase, toolLoadsExpectedSkill, toolReadsExpectedSkill };