...
// Source: skills/lumine-harness/src/harness/core/skill-catalog.ts
import { existsSync, readFileSync, readdirSync, realpathSync } from "node:fs";
import path from "node:path";
import { createHash } from "node:crypto";

//#region skills/lumine-harness/src/harness/core/skill-catalog.ts
...
function frontmatterField(source, field) {
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		if (![
			...
			">",
			">-"
		].includes(scalar)) return cleanScalar(scalar);
		const values = [];
...
function inspectSharedSkillCatalog(root) {
	...
	const skills = candidates.filter((skill) => !duplicateNames.includes(skill.name)).sort((left, right) => left.name.localeCompare(right.name));
	return {
		skills,
		diagnostics
	};
}
function discoverSharedSkills(root) {
	return inspectSharedSkillCatalog(root).skills;
}
function getSharedSkill(root, name) {
...
function searchSharedSkills(root, query = "", options = {}) {
	...
	return discoverSharedSkills(root).map((skill) => ({
		skill,
		score: scoreSkill(skill, terms)
	})).filter((item) => item.score > 0).sort((left, right) => right.score - left.score || left.skill.name.localeCompare(right.skill.name)).slice(0, limit).map((item) => item.skill);
}
function buildSharedSkillCatalog(root, options = {}) {
...
function sharedSkillReadFromTool(root, raw = {}) {
...
function sharedSkillLoadedFromTool(root, raw = {}) {
	const tool = String(raw.tool_name ?? raw.toolName ?? raw.name ?? "");
	...
	return discoverSharedSkills(root).slice().sort((left, right) => right.name.length - left.name.length).find((skill) => new RegExp(`(?:^|[^a-z0-9-])${skill.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:$|[^a-z0-9-])`, "i").test(text)) ?? null;
}

//#endregion
export { buildSharedSkillCatalog, discoverSharedSkills, getSharedSkill, inspectSharedSkillCatalog, searchSharedSkills, sharedSkillLoadedFromTool, sharedSkillReadFromTool };