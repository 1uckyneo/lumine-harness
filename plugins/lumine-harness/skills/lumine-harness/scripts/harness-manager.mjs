...
const CORE_MODULES = ["workflow", "generated"];
...
function relative(root, file) { return slash(path.relative(root, file)); }
...
function childGitRepositories(root) {
  const result = [];
...
export function inspectTarget(targetRoot) {
...
function selectedModules(inspect, requested = "auto") {
...
function parseProducts(value = "none") {
  if (value === "none" || !value) return [];
  const products = [...new Set(value.split(",").map((item) => item.trim()).filter(Boolean))];
  const unknown = products.filter((product) => !PRODUCTS.includes(product));
  if (unknown.length) throw new Error(`Unknown Adapter products: ${unknown.join(", ")}`);
  return products.sort();
}

function addTree(map, sourceRoot, targetRoot, filter = () => true) {
...
function sourceMap(inspect, adapters, modules) {
  const map = new Map();
  ...
  addTree(map, path.join(SKILL_ROOT, "assets", "harness"), ".harness", (file) => {
    ...
    return adapters.includes(product);
  });
...
export function createProposal(targetRoot, options = {}) {
...
function renderProjectTemplate(name, inspect) {
...
function backupFiles(root, pathsToBackup) {
...
function pruneEmptyParents(root, start) {
  let current = start;
...
export function applyProposal(proposalFile) {
  const proposal = readJson(path.resolve(proposalFile));
...
async function main() {
  ...
  if (command === "proposal" || (command === "upgrade" && args[0] === "--plan")) {
    ...
    const current = existsSync(path.join(target, ".harness", "project.json")) ? readJson(path.join(target, ".harness", "project.json")) : null;
