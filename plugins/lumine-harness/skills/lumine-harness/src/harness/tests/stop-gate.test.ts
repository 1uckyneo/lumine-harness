...
import { decideStopHookResponse, extractWorkStatus } from "../adapters/codex/hooks/lib/stop-gate.ts";
...
import { initializeSessionState, observeHarnessEvent, readSessionState, recordProgressObservation, recordUserTurn, setProgressObservability } from "../core/work-status.ts";
import type { HarnessHookInput, HarnessProduct, SessionState } from "../core/contracts.ts";
...
function requireSessionState(root: string, product: HarnessProduct, sessionId: string): SessionState {
...
function stopInput(
  ...
  extra: Partial<HarnessHookInput> = {}
): HarnessHookInput {
  return {
...
test("fresh status emissions can continue repeatedly until the configured chain limit", () => {
  ...
  try {
    ...
    recordUserTurn(root, { ...base, userTurnId: "new-user-turn", lastAssistantMessage: null });
...
test("two observable no-progress cycles pause while a progress event resets the count", () => {
  ...
  try {
    ...
    recordProgressObservation(root, { ...base, eventId: "write-complete", lastAssistantMessage: null });
...
test("semantic continuation and host delivery remain separate", () => {
  const request = { disposition: "request_continuation" } as const;
  const rejected = { disposition: "reject_completion" } as const;
