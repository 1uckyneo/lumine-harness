...
interface AdapterManifestItem {
  implementation?: unknown;
  setup?: string;
  setupActions?: AdapterSetupAction[];
  limitations?: string[];
  skills?: { mode?: string };
  continuation?: { delivery?: string; maxConsecutive?: number };
  capabilities?: Record<string, CapabilityEvidence>;
  maturity?: unknown;
  failMode?: unknown;
  repositoryTestedBridgeVersion?: string;
  [key: string]: unknown;
}
...
interface AdapterManifest {
  ...
  products?: Record<string, AdapterManifestItem>;
}
...
function has(relPath: string): boolean {
...
function prototypeEntries(yaml: string): PrototypeEntry[] {
  const lines = yaml.split(/\r?\n/);
  const entries: PrototypeEntry[] = [];
  let inPrototypes = false;
  let current: PrototypeEntry | null = null;
  let currentArray = "";
...
function safeDesignArtifactPath(artifactPath: string): string {
...
function checkDesignArtifact(base: string, artifactPath: string, label: string): void {
...
function checkDesignData(base: string, artifactPath: string, page: string): void {
  const normalized = safeDesignArtifactPath(artifactPath);
  if (!normalized) {
  ...
  if (/approval/i.test(String((meta as Record<string, unknown>).authority || ""))) {
...
function isDesignPrototypePath(relPath: string): boolean {
  return relPath.startsWith("docs/design-docs/") && /\/prototypes\/.+\.html$/i.test(relPath);
}

function fail(message: string, remediation: string): void {
  console.error(`FAIL: ${message}`);
  console.error(`Remediation: ${remediation}`);
  process.exitCode = 1;
}

function walk(dir: string, predicate: (file: string) => boolean = () => true, out: string[] = []): string[] {
...
function isTasteTarget(relPath: string): boolean {
...
function gitChangedFiles(cwd: string, prefix = ""): string[] {
  try {
...
function tasteCandidateFiles(): string[] {
...
function checkAgentsOperationalRules(agents: string): void {
...
function checkAdapters(): void {
  ...
  try {
    manifest = JSON.parse(read(".harness/adapter-capabilities.json")) as AdapterManifest;
  } catch {
  ...
  for (const product of ADAPTER_PRODUCTS) {
    ...
    for (const action of item.setupActions ?? []) {
      ...
      if (!action.id || !action.title || !Array.isArray(action.steps) || action.steps.length === 0 || action.steps.some((step) => typeof step !== "string" || !step.trim())
        || !action.successSignal || typeof action.reloadRequired !== "boolean" || !Array.isArray(satisfiedBy)
        || satisfiedBy.some((capability) => !ADAPTER_CAPABILITIES.includes(capability))) {
...
function checkFlatPlanningPaths(): void {
...
function checkGeneratedFile(file: string): void {
...
function checkDraft(target?: string): void {
  if (!target) return;
...
function checkDesign(target?: string): void {
  if (!target) return;
...
function checkPlan(target?: string): void {
  if (!target) return;
