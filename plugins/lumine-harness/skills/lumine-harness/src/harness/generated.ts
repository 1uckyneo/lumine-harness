...
const TARGETS = ["workspace-index", "repo-doc-index", "api-map", "db-schema", "frontend-routes", "frontend-components"] as const;
const SKIP = new Set([".git", "node_modules", "dist", "target", ".next", ".nuxt", ".output", "coverage"]);
type GeneratedTarget = (typeof TARGETS)[number];
...
function walk(dir: string, predicate: (file: string) => boolean = () => true, limit = 8000): string[] {
  const out: string[] = [];
  const visit = (abs: string): void => {
    if (!existsSync(abs) || out.length >= limit) return;
    for (const entry of readdirSync(abs, { withFileTypes: true })) {
      if (entry.name.startsWith(".")) {
        if (![".agents", ".harness", ".codex", ".claude", ".qoder", ".trae", ".cursor", ".opencode"].includes(entry.name)) continue;
      }
...
function metadata({ title, command, sourcePaths, completeness = "partial", notes = "" }: MetadataOptions): string {
  return [
    `# ${title}`,
    "",
    "> AUTO-GENERATED",
...
function writeTarget(target: GeneratedTarget, content: string): void {
  mkdirSync(OUT, { recursive: true });
  writeFileSync(path.join(OUT, `${target}.md`), `${content.trimEnd()}\n`, "utf8");
}

function hasBackendSignals(): boolean {
...
function generateWorkspaceIndex(command: string): string {
  ...
  return `${metadata({
    ...
    command,
    sourcePaths: ["AGENTS.md", "ARCHITECTURE.md", "docs/", ".agents/skills/", ".harness/", ".codex/", ".qoder/", ".trae/", ".cursor/", ".opencode/"],
    notes: "根级 harness 静态索引。"
  })}
${section("Root Entrypoints", bullets(existing(["README.md", "AGENTS.md", "ARCHITECTURE.md", ".harness/cli"])))}
...
function generateRepoDocIndex(command: string): string {
  ...
  return `${metadata({
    ...
    command,
    sourcePaths: ["AGENTS.md", "README.md", "CLAUDE.md", ".agents/skills/", "docs/"],
...
function generateApiMap(command: string): string {
...
function generateDbSchema(command: string): string {
...
function generateFrontendRoutes(command: string): string {
...
function generateFrontendComponents(command: string): string {
  if (!hasFrontendSignals()) {
...
function refresh(target: GeneratedTarget, command: string): void {
  const generators: Record<GeneratedTarget, (command: string) => string> = {
...
for (const item of selected) {
  if (!TARGETS.includes(item as GeneratedTarget)) {
    console.error(`Unknown generated target: ${item}`);
    process.exit(2);
  }
  refresh(item as GeneratedTarget, `./.harness/cli generated refresh ${target}`);
